...
Set `source: "auto"` for these automatic saves. Optionally set `context` to briefly note why this memory was created.
...
- "forget this" / "delete memory" → call `memory_delete` with the ID shown in brackets [#N]
- "update memory" → call `memory_update`
